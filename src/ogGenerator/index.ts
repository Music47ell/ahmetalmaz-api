...
export async function generateOg(
  ...
  function drawColoredWord(ctx: CanvasRenderingContext2D, text: string, word: string, x: number, y: number, normalColor: string, highlightColor: string) {
  const parts = text.split(new RegExp(`(${word})`, 'gi'))

  let currentX = x
  parts.forEach(part => {
    ctx.fillStyle = part.toLowerCase() === word.toLowerCase() ? highlightColor : normalColor
...
ctx.strokeRect(
  ...
  const lineY = descriptionStartY - index * 40
    drawColoredWord(ctx, line, "Türkiye", padding, lineY, colors.cullen, colors.turkyie)
  })
