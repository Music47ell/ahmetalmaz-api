...
export const getListenBrainzStats = async () =>
	withCache('listenbrainz:stats', 3600, async () => {
		...
		const [activityRes, listenRes, artistRes, albumRes, trackRes] =
			await Promise.all(Object.values(endpoints).map((url) => fetch(url)))

		const [activityJson, listenJson, artistJson, albumJson, trackJson] =
			await Promise.all(
				[activityRes, listenRes, artistRes, albumRes, trackRes].map((r) =>
					r.json(),
				),
			)
