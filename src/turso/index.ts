...
import { getFlagEmoji, decodeCfHeader, getCountryName } from '../utils/helpers.js'
...
.all()
.map((r) => ({ flag: r.flag || '🌍', country: r.country || 'Unknown', total: r.total }))
...
.all()
.map((r) => ({ flag: r.flag || '🌍', city: r.city || 'Unknown', total: r.total }))
...
.all()
.map((r) => ({ slug: r.slug || 'Unknown', title: r.title || 'Unknown', total: r.total }))
...
db
.query<{ total: number }, []>(`SELECT COUNT(*) as total FROM analytics WHERE ${monthlyBase}`)
.get()?.total ?? 0
...
const {
visitorId, sessionId, eventType, eventName,
title, slug, referrer,
browser, browserVersion, engine, engineVersion,
deviceType, deviceVendor, deviceModel,
language, os, osVersion, screenResolution, userAgent,
...
latitude, longitude, timezone, flag,
statusCode,
})
